      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          <div className="flex items-center">
            <p className="font-bold">{post.userId ? post.userId.userName : <span>Acount Deleted</span>}</p>
            <p className="text-xs">({formatTimeDiff(post.timestamp)})</p>
          </div>
          {post.fileUrl ? (
            <img src={post.fileUrl} alt="error" className="h-64 rounded-xl" />
          ) : (
            ""
          )}
        ...
        <div className="w-full">
          ...
          >
            ...
            <div className="text-secondary-blue text-sm">
              {hide ? <p>show comments...</p> : <p>hide comments...</p>}
            </div>
          </div>
          {hide ? (
            <div></div>
